...
def create_rtstruct(segmentations, uploaded_file, output_path):
    # Primeiro, convertemos o arquivo carregado para DICOM se necessário
    if uploaded_file is not None:
        if uploaded_file.name.endswith('.dcm'):
            # Se for DICOM, carregue diretamente
            original_ds = pydicom.dcmread(uploaded_file, force=True)
            patient_name = original_ds.PatientName if 'PatientName' in original_ds else "PAT_Name"
            patient_id = original_ds.PatientID if 'PatientID' in original_ds else "PAT_ID"
        else:
            # Se não for DICOM, crie um DICOM a partir do arquivo
            original_ds = create_dicom_from_non_dicom(uploaded_file)
            if original_ds is None:
                return None  # Se falhar na conversão, não faça nada
...
def create_dicom_from_non_dicom(uploaded_file):
    """ Função para converter arquivo não DICOM para um DICOM com metadados padrão. """
    ...
    if uploaded_file.name.endswith(".nii") or uploaded_file.name.endswith(".nii.gz"):
        ...
        dtype = np.uint8 if img_data.max() <= 255 else np.uint16  # Escolhe o tipo dependendo da intensidade
    ...
    # Associar o file_meta ao dataset
    ...
    ds.Modality = "OT"  # Outros (para imagem não DICOM)
...
with col1:
    ...
    # Armazena o nome do arquivo anterior para detectar mudanças
            ...
            if output_format == "PNG":
                ...
                try:
                    ...
                    with tempfile.NamedTemporaryFile(delete=False, suffix=".dcm") as tmp_rtstruct:
                        ...
                        rtstruct_output = create_rtstruct(segmentations, uploaded_file, tmp_rtstruct_path)
